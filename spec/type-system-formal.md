...
e & ::= & \mu\,x \mid c & \text{variable } (\mu \in \lbrace \varepsilon, \%, \mathord{\sim} \rbrace) \text{ / constructor} \\
  ...
  & \mid & @e & \text{force (of any expression)} \\
  & \mid & \&x & \text{borrow (of a variable)} \\
...
**Literal range.** Integer literals lex as *unsigned* digit strings; the surface form `-n` parses as unary-minus applied to `n`. A digit string that exceeds `i64::MAX` ($2^{63}-1 = 9223372036854775807$) is not a value on its own — the lexer emits an `integer literal out of range` error token. The sole admissible overflow is `i64::MIN`: when a unary `-` immediately precedes the out-of-range token for the magnitude $2^{63} = 9223372036854775808$, the parser folds the pair into the literal `i64::MIN` ($-9223372036854775808$) directly. The fold fires in both expression and pattern position, so `let x: i64 = -9223372036854775808` and the pattern `| -9223372036854775808 ->` are each well-formed as written — no const binding is required. The fold is keyed to the exact `i64::MIN` magnitude: a bare `9223372036854775808` with no leading `-` is a lex error (no preceding minus to fold into), and so is any larger magnitude such as `-9223372036854775809`. The underflowing form `let min_i64: i64 = -9223372036854775807 - 1` also still evaluates to `i64::MIN`, but is no longer necessary.
...
& & \text{where}~\Gamma' = \Gamma \setminus\!\!\setminus e_c,~C_k = \text{linConsumed}(\overline{s_k}, \Gamma'),~\text{lin}(\Delta) = \lbrace x \in \text{dom}(\Delta) \mid \Delta(x) = (1, S) \rbrace,~\text{div}(k) \Leftrightarrow \text{tail}(\overline{s_k}) = \bot\text{:} \\
& & L_\text{if} = \begin{cases} C_1 & \neg\text{div}(1) \wedge \neg\text{div}(2) \\ C_2 & \text{div}(1) \wedge \neg\text{div}(2) \\ C_1 & \text{div}(2) \wedge \neg\text{div}(1) \\ \text{lin}(\Gamma') & \text{div}(1) \wedge \text{div}(2) \end{cases} \\
& & \text{requires:}~C_1 = C_2~(\text{neither diverges});~C_\text{div} \supseteq C_\text{surv}~(\text{one diverges});~C_1 = C_2 = \text{lin}(\Gamma')~(\text{both diverge}) \\
\text{linConsumed}(\textbf{match}~e~\lbrace \overline{p_i \to s_i} \rbrace, \Gamma) & = & \text{linConsumed}(e, \Gamma) \;\uplus\; L_\text{match} \\
& & \text{where (}\Gamma_i~\text{extends}~\Gamma \setminus\!\!\setminus e~\text{with}~\text{bv}(p_i))~C_i = \text{linConsumed}(\overline{s_i}, \Gamma_i) \setminus \text{bv}(p_i),~N = \lbrace i \mid \text{tail}(\overline{s_i}) \neq \bot \rbrace,~\text{and} \\
& & L_\text{match} = \begin{cases} C_k~(\text{any}~k \in N) & N \neq \emptyset \\ \text{lin}(\Gamma \setminus\!\!\setminus e) & N = \emptyset \end{cases} \\
& & \text{requires}~\forall i, j \in N.\; C_i = C_j~(\text{the common survivor set}~J) \\
& & \text{requires}~N \neq \emptyset \Rightarrow \forall i \notin N.\; C_i \supseteq J~(\text{each divergent arm consumes}\geq J;~\text{early cleanup-before-exit allowed, leak forbidden}) \\
& & \text{requires}~N = \emptyset \Rightarrow \forall i.\; C_i = \text{lin}(\Gamma \setminus\!\!\setminus e)~(\text{all-divergent: every arm consumes all outer linears}) \\
...
Branch constructs require the linear-consumption sets of the **non-divergent** arms to be **equal** — call the common set $J$ — projected onto the outer environment, and credit $J$ as the branch's consumption. A **divergent** arm (one with $\text{tail} = \bot$) is held to a weaker but still non-trivial obligation: it must consume a **superset** of $J$. It may not consume *less* — dropping an outer linear that the surviving arms consume would leak it on the divergent path, since its resource is never released before control leaves the scope — but it *may* consume *more*, cleaning a still-live linear up early before its `throw` / `return`. That extra linear is **not** credited to $L$: it survives on the non-divergent paths and is consumed later there. (In the impl's "remaining" framing — outer linears left unconsumed by an arm — this is exactly $\text{div-remaining} \subseteq \text{join-remaining}$.) When **every** arm diverges there is no surviving join, so the rule tightens: each arm must consume the **same** set, and that set must be **all** outer linears live at the branch — the joinpoint is unreachable, so any still-live outer linear is a real leak — and that full set is credited.

This rejects the divergent-arm leak `if cond then throw E else consume(%a) end`: the divergent `then` arm consumes $\emptyset$, the surviving `else` arm fixes $J = \lbrace a \rbrace$, and $\emptyset \not\supseteq \lbrace a \rbrace$ — `%a` would leak on the `then` path. It *accepts* the previously-over-rejected `match x { C -> consume(%p); return | _ -> () }` (with `%p` consumed after the match): the surviving `_` arm fixes $J = \emptyset$, the divergent `C` arm consumes $\lbrace p \rbrace \supseteq \emptyset$ (early cleanup before its `return`), and `%p` — uncredited, hence still live — is consumed on the surviving path after the match. Each path consumes `%p` exactly once. Pattern-introduced linear binders are still required to be consumed within their arm regardless of divergence — a divergent arm's pattern still binds names that must be observably consumed before the arm's control-flow exit (the same way a divergent arm's body still type-checks its statements).

For **match**, the projection step is explicit: each arm's $\Gamma_i$ extends Γ with pattern binders (e.g.\ the $x$ in `Some(%x) ->`), and a linear pattern binder is required to be consumed *within its arm* rather than crossing into the outer-Γ equality check. Equivalently, $C_i = \text{linConsumed}(\overline{s_i}, \Gamma_i) \setminus \text{bv}(p_i)$ is the arm's consumption restricted to outer-Γ linears (which the equality / superset `requires` clauses compare), and the final `requires` clause discharges the per-arm obligation on $\Gamma_i \setminus \Gamma$ (the new bindings introduced by $p_i$). **if** has no pattern binders, so $C_k = \text{linConsumed}(\overline{s_k}, \Gamma')$ directly and the arm-consistency check is the two-arm form shown above.
...
\text{linConsumed}(\textbf{try}~\overline{s_t}~\overline{\textbf{catch}~p_i \to \overline{s_i}}~\textbf{end}, \Gamma) & = & L_\text{try} \\
& & \text{where}~\Gamma_\text{catch} = \Gamma \setminus\!\!\setminus \overline{s}_{\preceq\text{thr}},~\Gamma_i~\text{extends}~\Gamma_\text{catch}~\text{with}~\text{bv}(p_i), \\
& & \quad\Delta_\text{try} = \text{linConsumed}(\overline{s_t}, \Gamma) \setminus \text{linConsumed}(\overline{s}_{\preceq\text{thr}}, \Gamma),~\Delta_i = \text{linConsumed}(\overline{s_i}, \Gamma_i) \setminus \text{bv}(p_i), \\
& & \quad A = \lbrace i \mid \text{tail}(\overline{s_i}) \neq \bot \rbrace \cup \lbrace \text{try} \mid \text{tail}(\overline{s_t}) \neq \bot \rbrace,~\text{and} \\
& & L_\text{try} = \begin{cases} \text{linConsumed}(\overline{s_t}, \Gamma) & \text{try} \in A \\ \text{linConsumed}(\overline{s}_{\preceq\text{thr}}, \Gamma) \cup \Delta_j~(\text{any}~j \in A) & \text{try} \notin A \wedge A \neq \emptyset \\ \text{linConsumed}(\overline{s_t}, \Gamma) & A = \emptyset~(\text{unreachable canonical default}) \end{cases} \\
& & \text{requires}~\forall p, q \in A.\;\Delta_p = \Delta_q~\wedge~(A \neq \emptyset \Rightarrow \forall p \notin A.\;\Delta_p \supseteq \Delta_{q \in A})~\text{(mirrors }\href{#T-TryCatch}{\text{T-TryCatch}}\text{'s reachable-path agreement and its divergent-arm carve-out — nexus-kzo4s)}
\end{array}$$
</div>
...
Unification is symmetric: $\text{unify}(\tau_1, \tau_2) = \text{unify}(\tau_2, \tau_1)$ unless otherwise noted. The rules below are written with the "interesting" argument on the left; the symmetric case is implied. U-Borrow is intentionally asymmetric (no symmetric counterpart); [U-Row-Exn](#U-Row-Exn) is likewise directional (the `Exn` sentinel is admissible only in the expected/right position — a soundness requirement, see there); U-Expand is asymmetric in form but applies in both argument orders by the symmetry convention.
...
U-Row-Exn handles the variant-lattice subsumption of `Exn` described in §1.3: when the **expected** side carries the catch-all sentinel `Exn`, any constructors of `Exn` on the actual side are absorbed by it. This is the *only* place the type system admits a row-inclusion relationship — every other row rule requires entry-by-entry equality. **Unlike** the equality row rules, U-Row-Exn is *directional* and does **not** apply in both argument orders: `Exn` is admissible only in the expected position $\overline{\tau_2}$ (the right argument), following §Unification's left = actual / right = expected orientation at subsumption sites (T-Lambda's $\text{unify}(\rho_e^\text{body}, \rho_e)$, T-App's argument-vs-parameter throws check). A *specific* actual row $\lbrace \texttt{NotFound} \rbrace$ thus unifies with an expected $\lbrace \texttt{Exn} \rbrace$ (specific $\sqsubseteq$ catch-all — sound), but the reverse $\text{unify}(\lbrace \texttt{Exn} \rbrace, \lbrace \texttt{NotFound} \rbrace)$ — an actual row that may throw *anything* checked against a *narrower* declared row — must **fail**, and does: with no symmetric swap it falls through to [U-Row-Closed-Closed](#U-Row-Open-Open) and the multisets $\lbrace \texttt{Exn} \rbrace \neq \lbrace \texttt{NotFound} \rbrace$ disagree. Admitting that direction would let a body inferred to throw `{Exn}` satisfy a declared `throws { NotFound }`, so an exception outside the declared row escapes uncaught — a violation of **P5 (exception containment)**. It is intentionally restricted to closed-closed shapes; a shape where one side is an open row $\lbrace \overline{\tau} \mid {?}r \rbrace$ carrying (or paired with) `Exn` does **not** reduce to U-Row-Closed-Open before re-applying U-Row-Exn: U-Row-Closed-Open's premise $\overline{r_2} = \emptyset$ fails whenever the open side carries `Exn` (since $\overline{r_2} = \overline{\tau_2} \setminus \overline{c} \ni \texttt{Exn}$). In practice, by the time effect rows are unified against a closed call-site row all tail variables have been solved to closed rows, so the closed-closed form of U-Row-Exn always applies. The Exn-absorption check for throw sites that have not yet resolved an open tail is handled separately in the throw-site well-formedness pass (`row_admits_exception` in `src/typecheck/infer.nx`) rather than through row unification.
...
\text{true} & \text{if}~\tau = x\langle\overline{\sigma}\rangle \wedge x \in V \quad (\text{cycle: greatest fixpoint}) \\
...
Linear types ($\%\sigma$), functions, handlers, mutable refs ($\mathord{\sim}\sigma$), borrows ($\&\sigma$), thunks ($@\sigma$), and arrays ($[\lvert\,\sigma\,\rvert]$) have no defined value-equality at runtime; comparing them is a type error. (Identity comparison on these would be an explicit operator, not $==$.) **Unlike** `linear` / `autoDrop` — which take the *least* fixpoint (`false` on cycle, so a recursive type is non-linear unless a concrete component forces it) — $\text{comparable}'$ takes the **greatest** fixpoint: a named type already on the unfold path returns `true`, assuming the recursive occurrence is comparable and failing only when a *concrete* incomparable component (arrow, handler, $\mathord{\sim}\sigma$, $\&\sigma$, $@\sigma$, $[\lvert\sigma\rvert]$, or $\%\sigma$) appears on the recursion path. This is sound because structural `==` on a finite value terminates and the genuinely-incomparable forms are still rejected by their own arms; least-fixpoint here would be *wrong*, marking every recursive type incomparable. So a self-referential type whose recursion path has no inherently-incomparable field (e.g.\ a $\textbf{type}~\texttt{Tree}\langle T \rangle = \texttt{Leaf} \mathbin{\vert} \texttt{Node}(\ldots, \textit{left}: \texttt{Tree}\langle T \rangle, \ldots)$) is comparable iff $T$ is — matching the runtime `==`, which recurses through constructor tag + payload uniformly, and keeping the named-type path consistent with the list clause (so $[\tau]$ and the equivalent $\texttt{List}\langle\tau\rangle$ agree). The visited set is keyed by type **name** only, which is exactly right for *regular* recursion (List / Tree / mutually-recursive Even–Odd). For *non-regular* recursion that changes type-arguments at a deeper occurrence (e.g.\ a $\textbf{type}~\texttt{Nest}\langle T\rangle = \texttt{N}(\textit{inner}: \texttt{Nest}\langle\texttt{List}\langle T\rangle\rangle)$), name-only keying treats every depth as the same cycle, so $\text{comparable}'(\texttt{Nest}\langle\sigma\rangle)$ returns `true` regardless of $\sigma$. This is an **accepted, deliberate imprecision** (name+args keying would be needed for full precision): such non-regular recursive types have no finite inhabitants — every value would be infinitely deep — so there is no runtime value on which the over-approximate `true` could be observed, and the spec and impl (`cmp_visited` in `src/typecheck/infer/operators.nx`) agree to key by name. Both stay decidable (the name-keyed visited set bounds the unfold).
...
$$\dfrac{
  ...
  C_k = \text{linConsumed}(\overline{s_k}, \Gamma_b)\text{:}\quad C_1 = C_2~(\text{neither diverges});~C_\text{div} \supseteq C_\text{surv}~(\text{one diverges});~C_1 = C_2 = \lbrace x \mid \Gamma_b(x) = (1,S) \rbrace~(\text{both diverge}) \\[2pt]
...
$$\dfrac{
  ...
  C_i = \text{linConsumed}(\overline{s_i}, \Gamma_i) \setminus \text{bv}(p_i),~N = \lbrace i \mid \text{tail}(\overline{s_i}) \neq \bot \rbrace\text{:}\quad \forall i,j \in N.\,C_i = C_j;~~N \neq \emptyset \Rightarrow \forall i \notin N.\,C_i \supseteq C_{k \in N};~~N = \emptyset \Rightarrow \forall i.\,C_i = \lbrace x \mid \Gamma_b(x) = (1,S) \rbrace \\[2pt]
...
**Strip on per-arm signature match.** Each arm $e_j$ is a lambda discharged by [T-Lambda](#T-Lambda), whose own closure-linearization ($\tau_\to^\star$) wraps the arm in $\%$ whenever *that arm* captures a linear binding. The inferred $\tau_j$ is then $\%((\overline{\pi_j}) \to \kappa_j; \ldots)$, and a bare $\text{unify}(\tau_j, (\overline{\pi_j}) \to \kappa_j; \ldots)$ against the un-wrapped method arrow would fail — no U-rule peels a leading $\%$ during arrow unification. The per-arm premise therefore unifies $\text{strip}(\tau_j)$, peeling the closure-linearization $\%$ before the method-signature match, mirroring [T-Inject](#T-Inject)'s strip on the handler-pattern match. Without it, the very $\Gamma_\text{cap} \neq \emptyset \Rightarrow \%\tau_h$ machinery this rule carries to support linear-capturing handlers would be self-defeating: such a handler would have no derivation. (Impl divergence: `src/typecheck/check/registration.nx`'s `validate_arm_sig_go` does not infer-then-unify — it compares the arm's HIR-*declared* signature against the cap method by structural equality (`subst.type_eq`), never producing a $\%$-wrapped inferred type, so the impl avoids the hazard by construction rather than via `strip`.)
...
**Dispatch from T-Handler.** T-Continuation is reached through [T-Handler](#T-Handler)'s per-arm premise, which is a case split on arm shape: an arm with no continuation binder is typed as a pure lambda value via $\vdash_e e_j : \tau_j \mathbin{!} \lbrace\rbrace$ and matched to the method signature by $\text{unify}(\text{strip}(\tau_j), \ldots)$; an arm carrying `with @k` is instead discharged through $\vdash_\text{arm} e_j \mathbin{!} \beta_j$ (this rule). The two forms reconcile cleanly:

- **Arrow type.** The continuation arm never produces an inferred $\tau_j$ to feed back into `unify`; instead, T-Continuation's first premise reads $\text{methods}(x) \ni \ell_j : (\overline{\pi_j}) \to \kappa_j; \alpha_j; \beta_j$ directly, so the arm's declared parameters and return type are matched against the cap signature *inside* T-Continuation rather than by T-Handler's `unify`.
- **Effect.** T-Continuation's conclusion is $\mathbin{!} \beta_j$ — exactly the method's declared throw-row from the same first premise — so the per-arm effect is reconciled against the method signature by construction, and T-Handler needs no separate effect-unify for continuation arms.
- **Require row.** $\alpha_j$ flows into T-Handler's $\rho_\text{req} = \bigcup_j \alpha_j \cup \rho_\text{annot}$ uniformly: $\alpha_j$ comes from $\text{methods}(x)$ regardless of arm shape, so continuation arms contribute their require row identically to plain arms.

When the arm carries `with @k`, its body sees an extra linear binding $k :^{1} @\kappa_j$ in scope, and the arm's visible function type is the same $(\overline{\pi_j}) \to \kappa_j$ as declared on the cap — the presence of `with @k` does not change the method signature. The linearity of $k$ means the body may force the continuation at most once; any control path that forces $k$ more than once is rejected by the standard linear-usage check ([P-FnEnd](#T-Lambda)). Implementation: `src/typecheck/check.nx::check_arm_body` and `check_function` insert $k \mapsto \text{mono}(@\kappa_j)$ into the arm's body environment before running the standard statement-sequence check; `src/typecheck/check/registration.nx`'s `validate_arm_sig_go` strips the trailing continuation binder via `list_init` before matching params.
...
Borrowing does not consume the binding. Bare T-Borrow (on unrestricted $x :^{\omega}$) and the argument-position rule [T-App-BorrowLin](#T-App) (on linear $x :^{1}$) both use `stripBorrow` rather than `strip`, because borrowing a $\mathord{\sim}\tau$ binding should yield a $\&\tau$ view of the cell's contents, not a nonsensical $\&(\mathord{\sim}\tau)$ view of the cell itself. Like `strip`, `stripBorrow` leaves $@$ (thunk) in place: a thunk's result does not exist until [T-Force](#T-Force) runs it, so the suspended computation must be forced before its result can be viewed (write `let v = @x` then $\&v$). The premise $\text{unify}(\text{stripBorrow}(\tau'),\, \sigma)$ then reduces to $\text{unify}(@\sigma,\, \sigma)$ — a deferred result against a borrowed slot — which fails, so a borrow of an unforced thunk is rejected at the type level. The pattern-side `strip` instead keeps $\mathord{\sim}$ in place — refs cannot be match scrutinees — so the two operators agree on $\%$, $\&$, and $@$, parting only at the $\mathord{\sim}$ case.
...
$$\dfrac{
  ...
  \overline{s}_{\preceq\text{thr}} = \text{prefix of}~\overline{s_\text{try}}~\text{up to and including its first potential-throw-point statement} \\[2pt]
  \Gamma_\text{catch} = \Gamma \setminus\!\!\setminus \overline{s}_{\preceq\text{thr}} \quad\text{(catch-arm input:}~\Gamma~\text{minus the linears the try body consumes before its first possible throw — see below)} \\[2pt]
  ...
  \Delta_\text{try} = \text{linConsumed}(\overline{s_\text{try}}, \Gamma) \setminus \text{linConsumed}(\overline{s}_{\preceq\text{thr}}, \Gamma) \quad\text{(post-throw-point outer-linear consumption of try-success)} \\[2pt]
  \Delta_i = \text{linConsumed}(\overline{s_i}, \Gamma_i) \setminus \text{bv}(p_i) \quad\text{(outer-linear consumption of catch arm }i\text{)} \\[2pt]
  A = \lbrace i \mid \text{tail}(\overline{s_i}) \neq \bot \rbrace \cup \lbrace \text{try} \mid \text{tail}(\overline{s_\text{try}}) \neq \bot \rbrace \quad\text{(reachable post-construct paths)} \\[2pt]
  \forall p, q \in A.\;\Delta_p = \Delta_q \quad\text{(every reachable path agrees on the outer-linear residual, mirroring }\href{#T-Match}{\text{T-Match}}\text{)} \\[2pt]
  A \neq \emptyset \implies \forall p \notin A.\;\Delta_p \supseteq \Delta_{q \in A} \quad\text{(divergent paths consume at least the reachable-path residual — mirrors }\href{#T-Match}{\text{T-Match}}\text{'s}~C_i \supseteq C_{k \in N}~\text{carve-out; closes nexus-kzo4s)} \\[2pt]
  A = \emptyset \implies (\forall i.\;\Delta_i = \lbrace x \mid \Gamma_\text{catch}(x) = (1,S) \rbrace) \wedge \lbrace x \mid \Gamma(x) = (1,S) \rbrace \subseteq \text{linConsumed}(\overline{s_\text{try}}, \Gamma) \quad\text{(all-divergent: every catch arm consumes the full residual, try-body consumes every outer linear before its divergence)} \\[4pt]
  \Gamma_\text{out} = \begin{cases} \text{closeBlock}(\Gamma_1,\, \Gamma) & \text{if}~\text{try} \in A \\ \text{closeBlock}(\Gamma_i',\, \Gamma) & \text{if}~\text{try} \notin A \wedge i \in A~(\text{any such}~i\text{; equal by the agreement premise}) \\ \text{closeBlock}(\Gamma_1,\, \Gamma) & \text{if}~A = \emptyset~(\text{construct diverges; choice unreachable}) \end{cases} \\[4pt]
  ...
  \Gamma;\, \rho_q;\, \tau_r \vdash_s \textbf{try}~\overline{s_\text{try}}~\textbf{catch}~\overline{p_i \to s_i}~\textbf{end} : \Gamma_\text{out} \mathbin{!} \rho_\text{residual} \cup \textstyle\bigcup_i \rho_i
} \;\textsc{T-TryCatch}$$
</div>

The output environment $\Gamma_\text{out}$ is the **reachable-path** view of Γ. When the try body has a success path, that path's environment $\Gamma_1$ wins, restricted to Γ by `closeBlock` (dropping try-body locals). When the try body always diverges but at least one catch arm survives, any surviving arm's $\Gamma_i'$ wins; the $\Delta_p = \Delta_q$ agreement premise guarantees every choice yields the same outer-linear residual, so the rule is well-defined. When every path diverges, the construct itself diverges and the canonical default $\text{closeBlock}(\Gamma_1, \Gamma)$ is unreachable. `closeBlock` always strips bindings introduced inside the try body or a catch arm — pattern bindings on **let**, the catch pattern's own variables — so only outer-scope bindings flow through, matching lexical scoping.

**Catch arms see the try-body residual, not pre-try Γ.** Runtime linear consumption is **not** transactional: when a statement like $\text{consume}(\%a)$ runs and the try body subsequently $\textbf{throw}$s, the resource backing $\%a$ is already freed/moved — the $\textbf{throw}$ does not un-consume it. Typing the catch arm from the pre-try Γ would therefore be unsound: it would present $\%a$ as still available and let the catch arm consume it a second time (a double-free of a linear resource on the only reachable path `consume(%a)` → `throw` → catch `consume(%a)`). The fix is the residual base $\Gamma_\text{catch} = \Gamma \setminus\!\!\setminus \overline{s}_{\preceq\text{thr}}$: the linears the try body consumes *before its first possible throw* are removed before the catch arm is typed, so they are unavailable to the catch arm. $\overline{s}_{\preceq\text{thr}}$ is the statement prefix of $\overline{s_\text{try}}$ up to and including the first **potential throw-point** — a statement containing a function application, a $\textbf{throw}$, or a nested control construct (conservatively, since any of these may raise). Linears the try body consumes *only after* that point are still live for the catch arm: had a throw fired at-or-before the first throw-point, those later consumptions never ran. This precise throwing-prefix residual is *strictly more permissive* than removing every try-consumed linear, while still ruling out the double-free hazard — a linear consumed on every throwing path (i.e. before the first throw-point) cannot be re-consumed by the catch arm. Both the try-body and every catch arm carry the P-Block premise (the catch arm's against $\Gamma_\text{catch}$), ensuring no linear binding introduced inside either branch leaks past the construct.

The Δ-agreement premise unifies two cases the round-hjh2 rule got wrong (nexus-g50h3). When the try body has a success path, the catch arms must match $\Delta_\text{try}$ — the *post-throw-point* outer-linear consumption — since the pre-throw-point prefix is already removed from $\Gamma_\text{catch}$ on the throw path and is symmetrically subtracted out of $\Delta_\text{try}$ on the try-success side, so both paths have committed it; what remains is the outer linears the try-success tail consumed past the first throw-point, and a catch arm taking over from that throw point must consume the same set to leave Γ in the same shape. When the try body always diverges, only non-divergent catch arms contribute, and they must agree among themselves, mirroring [T-Match](#T-Match) / [T-If](#T-If) arm-equality. The round-hjh2 rule's two-sided gap was: (a) with a try that had a success path and a single catch arm consuming *fewer* outer linears, the catch-vs-catch equality was vacuous and the unconditional try-success output env declared linears consumed that the throw-then-catch path actually left live (a real leak admitted on paper); (b) with a try body that always throws, the same try-success output env declared linears as still live even though the (unreachable) success path was the only one claiming so — spuriously rejecting programs whose catch arm correctly cleaned them up. The branched $\Gamma_\text{out}$ plus the Δ-agreement on the reachable-path set $A$ closes both.

The two further premises — $A \neq \emptyset \implies \forall p \notin A.\;\Delta_p \supseteq \Delta_{q \in A}$ and the $A = \emptyset$ all-divergent clause — close the *divergent-arm leak* surfaced in nexus-kzo4s. The post-g50h3 Δ-agreement on $A$ alone left divergent paths unconstrained, so a try with a surviving body and a divergent catch arm consuming *fewer* outer linears than $\Delta_\text{try}$ trivially satisfied the agreement premise (catch arm not in $A$ → no equality obligation) while $\Gamma_\text{out} = \text{closeBlock}(\Gamma_1, \Gamma)$ unconditionally committed $\Delta_\text{try}$. Operationally, the throw path runs the divergent catch arm without consuming the leftover linears the try-body's success-path tail would have — the resource leaks before the catch arm's own divergence (e.g. via $\textbf{throw}$ propagation). The carve-out forces every divergent path to consume *at least* the reachable-path residual: when at least one path survives, divergent paths super-consume the survivor's $\Delta$; when every path diverges, every catch arm consumes the full residual $\lbrace x \mid \Gamma_\text{catch}(x) = (1, S) \rbrace$ and the try-body consumes every outer linear before throwing. The mirror with [T-Match](#T-Match)'s three-clause shape (`∀ i,j ∈ N. C_i = C_j; N ≠ ∅ ⇒ ∀ i ∉ N. C_i ⊇ C_{k∈N}; N = ∅ ⇒ ∀ i. C_i = lin(Γ_b)`) is exact: $A$ replaces $N$ and the construct's input Γ (offset by the pre-throw-point on the try side) replaces $\Gamma_b$.

**Spec ↔ impl.** The $\Gamma_\text{catch}$ derivation still matches the impl: `src/typecheck/linearity/walk.nx` types the catch arm from `filter_not_in(pre_try_vars, scan_stmts_until_throw(…))`, where `scan_stmts_until_throw` (`capture.nx`) accumulates outer-linear consumption left-to-right and stops at the first statement with a potential throw-point. The impl's "all calls are throw-points" convention is conservative: when a try-body call is provably pure it may stop the scan one statement early (admitting a marginally smaller $\Gamma_\text{catch}$); a precise throws-row check there is a future refinement. The arm-equality premise is realised in two phases: `try_body_diverges = stmts_diverge(b)` short-circuits when the try-body diverges and propagates the catch-arm's output state directly (`walk.nx:708-711`, landed in af35c4a8 — closes the case-(b) impl half of nexus-g50h3); otherwise `vars_equal(post_try_vars, post_catch_vars)` (`walk.nx:712`) enforces an equality stricter than the new $\Delta_p \supseteq \Delta_{q \in A}$ super-set carve-out — the impl over-rejects divergent-catch programs that the spec admits, which is safe (any equality on $A$ is a valid super-set on $\notin A$) but loses precision. Tightening the impl to the carve-out shape — replacing the post-try-vs-post-catch equality with the rule's $A$/$\notin A$ split — is the remaining impl refinement for nexus-kzo4s.
...
The catch-all condition `hasCatchAll` is *syntactic*: only an explicit wildcard arm $\_ \to \ldots$ or a single-variable arm $e \to \ldots$ (binding the exception value at type `Exn`) empties the residual row — a catch-all handles every variant **and** any unresolved open tail, so $\rho_\text{residual} = \lbrace\rbrace$ regardless of what $\rho_\text{try}$ contained. (The earlier formula $\rho_\text{try} \setminus (\lbrace\texttt{Exn}\rbrace \cup \overline{C}_\text{caught})$ was wrong: `caughtVariants` returns $\emptyset$ for a wildcard arm, and `Exn` is rarely a literal member of $\rho_\text{try}$, so set difference left the declared variants — e.g. $\lbrace\texttt{NotFound}, \texttt{PermDenied}\rbrace$ — apparently still throwable past a `_` arm that caught them, contradicting this paragraph's own claim.) A *closed enumeration* of currently-declared `Exn` variants does **not** empty the residual — `hasCatchAll` is false for it, so $\rho_\text{residual} = \rho_\text{try} \setminus \overline{C}_\text{caught}$ stands and any open tail ${?}r$ is forwarded unchanged. This is deliberate and makes **selective catch type-honest**: a `try`-`catch` whose arms list only specific variants (no $\_$ or single-variable arm) is **well-typed** — T-TryCatch imposes *no* catch-all precondition (contrast [T-Match](#T-Match)'s line for a bare `match` on an `Exn` *value*, where $\text{strip}(\tau) = \texttt{Exn} \implies \text{hasCatchAll}$ is required because a non-catch-all match has no residual channel and an unmatched value is a stuck dispatch) — and the variants the catch does not name remain in $\rho_\text{residual}$, propagating to the enclosing throws row. The cross-module extensibility concern is then a **recompile-discipline** property, not a type-system precondition: $\rho_\text{try}$ is re-inferred against the importer's current $\text{variants}(\texttt{Exn})$ on every build, so a downstream-added variant the try body can now throw reappears in $\rho_\text{try}$ — and thus in the residual — at the next compile; only a *stale* build (source unchanged, a dependency extended, not recompiled) could observe a mismatch, the same caveat every separately-compiled strongly-typed language carries. A syntactic catch-all is required only to **collapse** the residual to $\lbrace\rbrace$ and absorb the open tail.

**Spec ↔ impl.** The self-host typechecker matches this: it suppresses the `ExnNoCatchAll` diagnostic inside catch bodies (`src/typecheck/infer/exprs.nx`, gated on `state_in_catch_body`) and records the selective-catch rationale at `src/typecheck/types.nx` — an unmatched exception re-throws via WASM exception handling, the runtime counterpart of its staying in $\rho_\text{residual}$ and propagating up the declared throws row (nexus-3005q).
...
<div markdown="0">
$$\quad \exists~\textit{main} \in \text{dom}(\Gamma).\;\Gamma(\textit{main}) = (\omega,\, \text{mono}((\,) \to \texttt{unit};\,\rho_q;\,\rho_e)) \;\wedge\; \rho_e = \lbrace \overline{\eta} \rbrace \;\wedge\; \text{wfThrow}(\rho_e) \;\wedge\; \rho_q \subseteq \texttt{SysCaps} \;\wedge\; \textit{main} \notin \text{exports}(\mathcal{T})$$
</div>
...
- **Closed, wrappable throws row** — `main`'s declared $\rho_e$ is a **closed** row $\lbrace \overline{\eta} \rbrace$ (no tail row variable) all of whose entries satisfy $\text{wfThrow}$: each is a concrete `Exn` constructor, an exception group, or the catch-all sentinel `Exn`. The empty row $\lbrace\rbrace$ (a pure `main`) is the trivial case. Exceptions that escape user-`main` are not required to be discharged by an inner **try**: the HIR **main-wrap** pass synthesises a runtime entry wrapper around user-`main` that does a `match`-on-`Exn` with a wildcard arm, so any variant in $\rho_e$ has a consumer outside user code. Rejected (via `MainThrowsTooBroad`) are exactly the rows the wrapper cannot route: an **open** row $\lbrace \overline{\eta} \mid {?}r \rbrace$ (a rest tail could carry unbounded unknown variants), a row that has collapsed to a free type variable, and any entry that is not a named exception or group.
...
A program is **well-formed** iff $\text{wfMain}(\mathcal{T})$ holds for the root module's resolved $\mathcal{T}$. If the property fails — `main` is missing, has the wrong signature, declares a non-wrappable throws row (open, free-variable, or non-exception entries), requires a non-system capability, or is exported — the type-checker rejects the program at the program-entry check after the per-module folds complete. This is the only program-global property in §3; every other rule and property is module-local.
