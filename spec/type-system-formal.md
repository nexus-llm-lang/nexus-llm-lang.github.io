...
& & \text{requires}~\forall p, q \in A.\;\Delta_p = \Delta_q~\text{(mirrors }\href{#T-TryCatch}{\text{T-TryCatch}}\text{'s reachable-path agreement)}
\end{array}$$
</div>
...
$$\dfrac{
  ...
  \forall p, q \in A.\;\Delta_p = \Delta_q \quad\text{(every reachable path agrees on the outer-linear residual, mirroring }\href{#T-Match}{\text{T-Match}}\text{)} \\[4pt]
