...
**Literal range.** Integer literals are lexed as *unsigned* digit strings; the surface form `-n` is parsed as unary-minus applied to the literal `n`. The lexed digit string must fit in `i64::MAX` ($2^{63}-1 = 9223372036854775807$). The boundary value `i64::MIN` ($-2^{63} = -9223372036854775808$) therefore has **no direct literal form** — writing `-9223372036854775808` lexes the unsigned digits `9223372036854775808`, which exceeds `i64::MAX` by one and is rejected with `integer literal out of range`. The workaround is to write the value as an expression that stays within range at each sub-literal: `let min_i64: i64 = -9223372036854775807 - 1` (the subtraction underflows by design). This is a deliberate spec choice — accepting the unsigned digit string `9223372036854775808` *just* for the unary-minus context would require lexer-parser coupling on context, which the impl deliberately avoids; the constant-folding workaround is one statement long and unambiguous. (Pattern position has the same constraint: `match n { -9223372036854775808 -> ... }` is rejected and must be rewritten using a const binding.)
...
$\text{linear}(\tau)$ is a structural (recursive) predicate: holds if τ is $\%\sigma$, $@\sigma$, or $[\lvert\,\sigma\,\rvert]$ at the outermost level, or if any transitive component of τ is linear. Components inspected by the recursion: fields of records, type arguments of named types, element types of lists, and elements of rows. Borrows $\&\sigma$ and mutable refs $\mathord{\sim}\sigma$ are *not* recursed into: a borrow is always reusable ($q = \omega$), so the borrow itself is non-linear regardless of σ; the underlying owner's linearity is tracked at the owner's binding. A mutable ref's payload would be a counterpart consideration, but [§Mutable Reference Well-Formedness](#mutable-reference-well-formedness) rejects $\mathord{\sim}\sigma$ at every introduction site whenever $\text{linear}(\sigma)$ — by the time `linear` is consulted on a $\mathord{\sim}\sigma$ value the payload is already known non-linear, so the wrapper contributes nothing to overall linearity. Function arrows $(\overline{\ell:\tau}) \to \tau_r$ and handlers $\textbf{handler}\;x\;\rho$ are values whose own linearity is governed by capture-linearisation in [T-Lambda](#T-Lambda) / [T-Handler](#T-Handler) rather than by transitive recursion; `linear` does not look inside them. Example: $\text{linear}(\text{Pair}\langle\%\texttt{i64}, \texttt{i64}\rangle)$ holds; $\text{linear}(\mathord{\sim}\texttt{i64})$ does not (the $\mathord{\sim}$ wrapper is structurally non-linear).
