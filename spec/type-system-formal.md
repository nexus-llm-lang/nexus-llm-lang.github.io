...
$$\dfrac{
  ...
  \forall j \in J.\;\text{unify}(\text{strip}(\tau_j),\;(\overline{\pi_j}) \to \kappa_j;\, \alpha_j;\, \beta_j) \quad\text{(arm signature absorbs declared method type; \text{strip} peels the arm's closure-linearization \%; row premises invoke [U-Row-Exn](#U-Row-Exn) as needed)} \\[2pt]
