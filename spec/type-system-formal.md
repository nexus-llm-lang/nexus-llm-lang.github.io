...
& & \text{}\href{#T-TryCatch}{\text{T-TryCatch}}\text{ types catch arms from the throwing-prefix} \\
& & \text{residual}~\Gamma \setminus\!\!\setminus \overline{s}_{\preceq\text{thr}}\text{, so a catch arm cannot re-consume a linear the}\\
& & \text{try body consumed before its first possible throw; the catch arm's own}\\
& & \text{residual consumption is not threaded into this clause — consistent with}\\
& & \text{the try-success output view)}
\end{array}$$
</div>
