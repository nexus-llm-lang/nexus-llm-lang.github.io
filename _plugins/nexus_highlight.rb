...
module NexusHighlight
  ...
  # stay at the top level, but bare fn signatures get wrapped in a port block.
  def self.prepare_input(code)
  lines = code.split("\n")
  has_real_code = lines.any? { |l| l.strip.match?(/\A(port|handler|let|import|inject|match|if|for|while|try|conc)\b/) }
  ...
  if wrapped
    # Remove the "port Sig_ do" and "end" wrapper lines, and the blank line before the port
    # Find and remove wrapper lines by content
    lines.reject! { |l| l.match?(/<span class='keyword'>port<\/span>.*Sig_/) || l.strip == "<span class='keyword'>end</span>" }
